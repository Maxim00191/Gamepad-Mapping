...
public interface IKeyboardEmulator
{
    ...
    void KeyUp(Key key);
    void TapKey(Key key, int repeatCount = 1, int interKeyDelayMs = 0, int keyHoldMs = 70);
    Task TapKeyAsync(Key key, int repeatCount = 1, int interKeyDelayMs = 0, int keyHoldMs = 70, CancellationToken cancellationToken = default);
    /// <summary>Modifier keys down (in order), main key tap, modifiers up (reverse order).</summary>
    void TapKeyChord(IReadOnlyList<Key> modifiers, Key mainKey, int keyHoldMs = 70);
    Task TapKeyChordAsync(IReadOnlyList<Key> modifiers, Key mainKey, int keyHoldMs = 70, CancellationToken cancellationToken = default);
