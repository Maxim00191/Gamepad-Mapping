...
[GitHub Actions](.github/workflows/build.yml) runs on every push and pull request to `main` (`dotnet build` Release). Pushing a tag matching `v*` (e.g. `v1.2.0`) **publishes** two `win-x64` artifacts and creates a **GitHub Release** ([softprops/action-gh-release](https://github.com/softprops/action-gh-release)):
