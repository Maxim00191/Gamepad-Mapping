...
| `output.keyboard_key` | Keyboard key | Sends a keystroke via the emulation stack. | `flow.in (Execution, triggers key action)`, `probe.image (ImageOrCoordinates, optional context image)` | `flow.out (Execution, emitted after key action)` |
| `output.mouse_click` | Mouse click | Injects a mouse button click. | `flow.in (Execution, triggers mouse click)`, `probe.image (ImageOrCoordinates, optional context image)`, `coord.x (Number, click target X)`, `coord.y (Number, click target Y)` | `flow.out (Execution, emitted after click)` |
