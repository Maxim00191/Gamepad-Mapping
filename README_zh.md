...
当推送匹配 `v*` 规则的标签（如 `v1.0.0`）时，流水线会额外构建**独立运行 (Self-contained)** 的 `win-x64` 版本，将其打包为 zip 文件，并通过 [softprops/action-gh-release](https://github.com/softprops/action-gh-release) 自动创建包含该附件的 **GitHub Release**。
